...
<li><a href="member.php">Next 7 days</a></li>
	...
	if (isset($_POST['submit']))
    {
                        ...
                        if(strtotime($time)>=strtotime($time0) && strtotime($time)<=$time1 && $row['eventtype']==$venue)
                        {
                            ...
                            <center>
                                <div class='alert' style='width:450px;'>
                                    An event already exists from ".date('h:i A',strtotime($time0))." to ".date('h:i A',$time1)."!<a href='manage.php'> unBook again</a>
                                </div>
                            </center>
                            ");
                        }
		...
		$querypart = mysql_query("DELETE FROM participantinformationTable where uname='$username' and eventid = $id");
		...
		die('<center><div class="alert alert-success" style="width:250px;">
                    <strong>Unbooking successful!</strong>
                    </div></center>');
            }      
        }
        else
...
<center>
    ...
    <form action="unbook_event.php" method='POST' id="addBooking">
    <table>
        <tr>
...
<option value="chenai">chennai</option>
        ...
        <tr>
            <td colspan="2">
            <center><button type="submit" class="btn" name="submit">Unbook</button></center>
        </td>
